            ...
            <table id="dataTable" class="table-ui w-full">
                <thead>
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left">Mata Pelajaran & Kelas</th>
                    ...
                    <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="inline-flex items-center justify-center gap-2">
                                ...
                                <a href="{{ route('grades.edit', \$grade) }}" class="p-2 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-500 hover:text-white transition" title="Edit Nilai">
                                    <i class="fas fa-edit text-sm"></i>
                                </a>
                                <form action="{{ route('grades.destroy', \$grade) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data nilai ini?')" class="inline">
                                    @csrf
                                    @method('DELETE')
...
<script>
    $(document).ready(function() {
        $('#dataTable').DataTable({
            ...
            responsive: true,
            order: [[ 1, "asc" ]] // Order by Student Name default
        });
    });
</script>
@endpush
...
file_put_contents('c:/laragon/www/sistem-e_learningV1/resources/views/grades/index.blade.php', $code);
echo "Berhasil update grades views.";
