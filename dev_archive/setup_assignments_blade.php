    ...
    <div class="card">
        <div class="card-header border-b-0 pb-0">
            <div>
                ...
                <p class="page-subtitle">Manajemen penugasan, batas waktu, dan pemantauan pengumpulan.</p>
            </div>
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="inline-flex items-center justify-center gap-2">
                                <a href="{{ route('assignments.show', \$assignment) }}" class="p-2 rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-800 hover:text-white transition" title="Lihat/Nilai">
                                    <i class="fas fa-eye text-sm"></i>
                                </a>
                                @if(auth()->user()->isTeacher() || auth()->user()->isSuperAdmin())
                                <a href="{{ route('assignments.edit', \$assignment) }}" class="p-2 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-500 hover:text-white transition" title="Edit">
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-16 text-center text-slate-400">
                            ...
                            @if(auth()->user()->isSuperAdmin() || auth()->user()->isTeacher())
                            <div class="mt-4">
                                <a href="{{ route('assignments.create') }}" class="btn btn-primary">Buat Tugas Pertama</a>
                            </div>
                            @endif
                        </td>
                    </tr>
...
<script>
    $(document).ready(function() {
        $('#dataTable').DataTable({
            ...
            responsive: true,
            order: [[ 3, "desc" ]] // Order by deadline default
        });
    });
</script>
@endpush
...
file_put_contents('c:/laragon/www/sistem-e_learningV1/resources/views/assignments/index.blade.php', $code);
echo "Berhasil update assignments views.";
